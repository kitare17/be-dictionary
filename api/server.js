...
server.get('/gemini', async (req, res) => {
    ...
    const INSTRUCTION = `
        ...
        1/Special cases
        1.1/ AFFRICATES - Never split
        They are always a single consonant, not a group of two sounds: /tʃ/, /dʒ/
        1.2/ DIPHTHONGS - Never split into two vowels
        - a single nucleus, not two syllables: /eɪ/, /aɪ/, /ɔɪ/, /aʊ/, /əʊ/, /oʊ/, /ɪə/, /eə/, /ʊə/.
        1.3/ LONG VOWELS, TENSE VOWELS — DO NOT SPLIT

        2/ The strongest rule - Short vowel constaint
        2.1/ Only applies to: Short vowels /ɒ, æ, ʊ, ɪ, ɛ/ carry primary stress
            Syllable containing short vowel stressed → must be closed (CVC)
        2.2/ How to choose a coda: Select the maximum valid coda (single, double, triple). The consonant remainder (if any) is pushed to the onset of the next syllable, if valid.     
        2.3/ Purpose: Keep short vowels “checked”, avoid prolongation
        2.4/ Example
            /ˈkæ.tə/ → /ˈkæt.ə/ (because /æ/ is stressed → must close)
            /ˈæ.tləs/ → /æt.ləs/ (coda /t/)

        3/ Additional Rule – Stressed Open Vowel Constraint (SOVC)	
        3.1/ Only applies to: Open/long vowels (open vowels /ɑː, ɜː, ɔː/) with main stress
            Syllable containing stressed open vowel → should close syllable
        3.2/ How to choose coda: Choose the maximum valid coda (single, double, triple). Consonant residue pushes to the onset of the next syllable, if valid
        3.3/ Example:
            /ˈfɑː.mə/ → /ˈfɑːm.ə/ (coda /m/ do SOVC)
            /ˈɜː.rɪŋ/ → /ˈɜːr.ɪŋ/ (coda /r/)

        4/ Maximal Onset Principle (MOP) 
        4.1/ Applicable when: Vowel is not short vowel or short vowel but not primary stressed
        ...
        5/ General notes when dividing
        5.1/ If the coda is already selected but there is a consonant → push to the next syllable onset, if valid
        5.2/ Unstressed syllables → MOP applies normally
        5.3/ /r/ at the end of syllable 2 → depends on American or British accent

        6/ Special notes on /r/
        6.1/ British English (RP, non-rhotic)
        6.1.1/ RP transcription still writes /r/ when written as /kɑː(r)/ to reflect spelling and linking /r/.
        6.1.2/ In the CODA table and O–N–C tree diagram, /r/ is not considered a coda if:
        ...
        6.2/ American accent (AmE, rhotic)
        6.2.1/
        ...
        7/ Stressed Diphthong Rule (SDR)
        7.1/ Common diphthongs:
        ...
        Also a single nucleus: /iː/ /uː/ /ɑː/ /ɔː/ /ɜː/ (The “ː” mark is not a separate sound, so it should not be split.)
        Return a json following format
            [
                {
                  "index": 1,
